...
template <class T>
inline void thrust_reduce(
    thrust::device_vector<T>& d_input, 
    thrust::device_vector<T>& d_sums, 
    int n, int k){ 
...
int main(int argc, char **argv){
    ...
    do {
        ...
        thrust_reduce<float>(d_data_x, d_centroids_x, n, k);
        thrust_reduce<float>(d_data_y, d_centroids_y, n, k);
        thrust_reduce<float>(d_data_z, d_centroids_z, n, k);
        thrust_reduce<int>(d_closest, d_sums, n, k);
